...
Floating preview clicks pass through `SettingsStore.activateOnOverlayClick`. The main grid always activates a selected window, while floating panels can be configured to select without activating. Overlay panels accept the first click without becoming key, never render the main workspace's selected state, and explicitly leave Vantage inactive after switching so another preview can switch back with one click. Compact overlay cards expose persisted size and opacity presets through their context menu, and a drag that moves beyond the click threshold swallows its mouse-up so the hosted button never fires a window switch after a panel move. Each panel saves its origin under a stable application, exact-title, and session-pinned instance key (geometry is deliberately excluded, so resizing a game window never changes the key), restores that origin on launch, and clamps it to the current screen when display geometry changes. A panel whose window vanishes stays hidden in a short grace limbo instead of closing: when the same identity reappears under a new window ID (fullscreen toggles, transient empty titles), the panel is rebound in place rather than recreated at a default slot. Instance ordinals come from `WindowOrdinalAllocator`, which pins ordinals per window ID for the session and tombstones vanished members briefly, so a one-cycle dropout cannot reshuffle the surviving windows' keys.
